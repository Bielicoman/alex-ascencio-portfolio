...
export default {
  ...
  mount(host) {
    ...
    const nearest = (h) => {
      const hit = S.pick(h.x, h.y, meshes);
      if (hit) { let o = hit.object; while (!objs.includes(o)) o = o.parent; return o; }
      // tolerância: objeto cuja projeção está a menos de 70 px
      let best = null, bd = 70;
      for (const o of objs) { const p = S.toScreen(o.position), d = Math.hypot(p.x - h.x, p.y - h.y); if (d < bd) { bd = d; best = o; } }
      return best;
    };
    ...
    return {
      frame(dt, hands) {
        ...
        for (const h of hands) {
          if (h.down) {
            ...
            if (o && ![...grabs.values()].some((g) => g.o === o && g.pair)) {
              ...
              if (other) { // segunda mão no mesmo objeto → escala
                ...
                const w = S.toWorld(h.x, h.y, o.position.z);
                grabs.set(h.id, { id: h.id, o, off: o.position.clone().sub(w), z0: o.position.z, sc0: h.scale, a0: h.angle, r0: o.rotation.z });
                o.userData.grab = h.id;
                A.pop(0.25);
              }
            }
          }
          if (h.up && grabs.has(h.id)) {
            ...
            if (!g.scaleOf) {
              ...
              g.o.userData.grab = null;
              const sp = g.o.userData.v.length();
        ...
        // mãos segurando: mola crítica até o alvo (sem teleporte, sem tremor)
        for (const g of grabs.values()) {
          ...
          // profundidade pelo tamanho aparente da palma (câmera) — mais perto = maior
          const z = h.src === "cam" ? Math.max(-6, Math.min(4, g.z0 + (h.scale / g.sc0 - 1) * 9)) : g.z0;
          S.toWorld(h.x, h.y, z, tmp).add(tmp2.set(g.off.x, g.off.y, 0));
          tmp.z = z;
          const k = 1 - Math.exp(-dt * 18);
          const nv = tmp.clone().sub(o.position).multiplyScalar(k / Math.max(dt, 1e-3));
          u.v.lerp(nv, 0.6);
          o.position.lerp(tmp, k);
          if (h.src === "cam") o.rotation.z += ((g.r0 - (h.angle - g.a0)) - o.rotation.z) * k;
          u.w.set(-h.vy * 0.0008, h.vx * 0.0008, 0);
        }
        // física livre: amortecimento, paredes, colisões esféricas
        for (const o of objs) {
          const u = o.userData;
          o.scale.setScalar(o.scale.x + (u.s - o.scale.x) * Math.min(1, dt * 10));
          if (u.grab) { o.rotation.x += u.w.x; o.rotation.y += u.w.y; continue; }
          ...
          for (const [ax, lim] of [["x", wx], ["y", wy], ["z", 3.5]]) {
            const lo = ax === "z" ? -6 : -lim;
